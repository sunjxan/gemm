...
// thread_unit应能整除block_unit，warp_n和warpSize/warp_n应能整除block_dim
constexpr size_t thread_unit = 1, warp_n = 4;

__global__ void kernel(const real (*A)[K], const real (*B)[N], real (*C)[N])
{
    unsigned tid = threadIdx.x, warp_line = block_dim / warp_n * warpSize;
